  ...
  <div>
    ...
    <v-app-bar color="primary" flat border="bottom">
      ...
      <v-btn
        ...
        variant="flat"
        color="teal-darken-3"
        class="mr-4 rounded-pill font-weight-bold"
        @click="showHistory = true"
      >
        RECALL
        <!-- v-if="bumpedHistory.length > 0"
        :content="bumpedHistory.length" -->
        <v-badge color="error" inline></v-badge>
      </v-btn>
      <!-- <v-chip
        :color="kitchenStore.isConnected ? 'success' : 'error'"
        class="mr-4"
      >
        <v-icon start size="14">
          {{ kitchenStore.isConnected ? 'mdi-wifi' : 'mdi-wifi-off' }}
        </v-icon>
          ...
          >
            ...
            >
              <template #item="{ element }">
                <OrderCard :order="element" @served="markServed" />
              </template>
            </draggable>
          </v-col>
        </v-row>
      </v-container>
      <OrderHistory v-model="showHistory" />
    </v-main>
  </div>
</template>
...
<script setup>
  ...
  import OrderHistory from '../../components/kitchen/OrderHistory.vue'
  ...
  const markServed = async orderId => {
    await kitchenStore.markServed(orderId)
  }
